...
```{r echo=F}
library(tidyverse)
author_voices <- rio::import("author_voices/acoustics.xlsx")
...
To hear different voice pitches and formants, click the initials of the authors of our study below. You'll hear each of us speak the letters "A-E-I-O-U". Our 
...
data <- vcs %>% full_join(author_voices %>% mutate(voice_id=1e5+row_number()), suffix = c("_vcs", ""), by = "voice_id")
data$x <- data$f0
data$y <- data$pf
